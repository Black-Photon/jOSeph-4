...
public class MessagingFiles extends Files {
	...
	public ArrayList<Connection_Data> readMessagingConfig(String location){
		try {
			...
			new Error("Error #0017: IOException at MessagingFiles.java", 600).showModalWindow();
			e.printStackTrace();
		}
		return null;
	}
	...
	public void saveMessagingConfig(String location, ArrayList<Connection_Data> data){
		...
		try {
			...
			new Error("Error #0018: IOException at MessagingFiles.java", 600).showModalWindow();
			e.printStackTrace();
		}
	}
	/**
				...
				case ';': //Set the port and sets the data, adding it to the ArrayList
					if(count==2) {
						try {
							...
							new Error("Port from messaging_config.txt in wrong format",600).showModalWindow();
							e.printStackTrace();
						}
					}
