...
public class Calculator_Controller {
	...
	private void parseAnswer(){
		...
		if(operators.size()!=numbers.size()){
			Error error = new Error("Error", 450);
			error.setLabel("Syntax Error: Too many/few operators");
			return;
		}
		...
		if(numbers.size()==1) {
			...
			error.setLabel("Math Error: No Input");
		}else{
			Error error = new Error("Error", 300);
			error.setLabel("Math Error: Unused numbers");
		}
	}
	...
	private ArrayList<ArrayList> splitInfo(String string){
		...
		ArrayList<Character> operatorArray = new ArrayList<Character>();
		...
		for(char character: string.toCharArray()){
			if(isOperator(character)){
				numberArray.add(parseNumber(temp));
				temp = "";
	...
	private float performOperation(char operator, float num1, float num2){
		switch(operator){
			...
			default:
				Error error = new Error("Error", 300);
				error.setLabel("Operation Error");
				return 0;
		}
	}
	...
	private float parseNumber(String string){
		try{
			...
			for(char character: string.toCharArray()){
				if(beforeDecimal) {
					...
					if(character=='.'){
						...
						error.setLabel("Syntax Error: 2 decimals not allowed");
						return 0;
					}
			...
			if(!temp.equals("") && !beforeDecimal) {
				afterDecimal = Integer.parseInt(temp);
