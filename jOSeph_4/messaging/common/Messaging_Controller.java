...
import jOSeph_4.messaging.common.connectionSettings.Connection_Settings;
...
import jOSeph_4.messaging.client.Client;
import jOSeph_4.messaging.server.Server;
...
public class Messaging_Controller implements Initializable{
	...
	@FXML void onPressJoin() {
		...
		if(clientside()){
			Client.setData(data);
			Main.createWindow("Client.fxml" ,Variable.getWindow(), "Messaging");
		}else
		if(serverside()){
			Server.setData(data);
			Main.createWindow("Server.fxml" ,Variable.getWindow(), "Messaging");
		}
